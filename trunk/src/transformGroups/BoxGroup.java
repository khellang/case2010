...
public class BoxGroup extends TransformGroup {
	...
	public BoxGroup(int slideTime, BoundingSphere bounds, TextureList textures, Point3f lightPos, Point3f wallPos, int delay) {
		...
		transG.addChild(randomRotGroup);
		...
		ScaleInterpolator scaler = new ScaleInterpolator(scaleAlpha, scaleGroup);
	...
	class ShadowUpdater implements GeometryUpdater {    
		public void updateData(Geometry geometry) {
			...
			for (int i = 0; i < n; i++) {
				...
				Point4d p4 = new Point4d(v4);
				p.project(p4);
				boolean cutShadow = true;
				if (p.x > 1.5 && cutShadow)
					vert[3*i] = 1.5;
				else if (p.x < -1.5 && cutShadow)
					vert[3*i] = -1.5;
				else
					vert[3*i] = p.x;
				if (p.y > 1 && cutShadow)
					vert[3*i+1] = 1;
				else if (p.y < -1 && cutShadow)
					vert[3*i+1] = -1;
				else
					vert[3*i+1] = p.y;
				vert[3*i+2] = p.z;
